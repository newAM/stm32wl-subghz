...
use stm32wl::stm32wl5x_cm4::{self as pac, Peripherals};
...
impl SubGhz {
    ...
    ///
    /// This will initialize the SPI bus for you, but it will **not** bring the
    /// radio out of reset, or enable the clocks on the radio.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub unsafe fn conjure() -> SubGhz {
        let pac: Peripherals = Peripherals::steal();
        SubGhz { spi: pac.SPI3 }
    }
    ...
    /// ```no_run
    /// # let mut sg = unsafe { subghz::SubGhz::conjure() };
    /// use subghz::{CfgDioIrq, Irq, IrqLine};
    ///
    ...
    /// ```no_run
    /// # let mut sg = unsafe { subghz::SubGhz::conjure() };
    /// use subghz::{GfskBandwidth, GfskBitrate, GfskFdev, GfskModParams, GfskPulseShape, PacketType};
    ///
    ...
    /// ```no_run
    /// # let mut sg = unsafe { subghz::SubGhz::conjure() };
    /// use subghz::{CodingRate, LoRaBandwidth, LoRaModParams, PacketType, SpreadingFactor};
    ///
